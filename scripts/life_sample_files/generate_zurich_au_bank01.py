...
RUN_DATE_END: date = date(2026, 6, 18)
N_RUN_DAYS:   int  = 30
...
def derive_life3_batches() -> dict:
    """Volume-scaled Life3 BATCH values per merchant. Count mirrors
    derive_n_batches; values step by merchant count so ranges never collide
    (each merchant's life3_batch_base stays its first batch)."""
    n_batches = derive_n_batches()
    stride = len(MERCHANT_WEIGHTS)
    return {
        m: [f"B{MERCHANT_META[m]['life3_batch_base'] + k * stride}"
            for k in range(n_batches[m])]
        for m in MERCHANT_META
    }
...
def build_txns(rng: random.Random):
    ...
    txn_counts      = derive_txn_counts()
    n_batches       = derive_n_batches()
    life3_batches   = derive_life3_batches()
    ...
    for merchant, types in type_counts.items():
        ...
        for seq_in in range(1, types["payment"] + 1):
            ...
            life3_batch = rng.choice(life3_batches[merchant])
            gdpp_trans_desc = (
                f"{meta['first_letter']}{seq5}{row_yymmdd}{row_yymm}"
                f"{meta['k_code']}{life3_batch}{RUN_NONCE}"
            )
            ...
            t = Txn(
                ...
                life3_batch=life3_batch, gdpp_trans_desc=gdpp_trans_desc,
...
def main():
    ...
    print(f"[config] life3 batches    = {derive_life3_batches()}")
